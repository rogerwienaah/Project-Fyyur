...
#associative table for the m2m relationship b/n artist and genre
artist_genre_table = db.Table('artist_genre_table',
    db.Column('genre_id', db.Integer, db.ForeignKey('Genre.id'), primary_key=True),
    db.Column('artist_id', db.Integer, db.ForeignKey('Artist.id'), primary_key=True)
)

#associative table for the m2m relationship b/n venue and genre
...
class Venue(db.Model):
    ...
    #link venue and show -- one2many relationship
...
class Artist(db.Model):
    ...
    #link artist and show -- one2many relationship
...
class Show(db.Model):
    ...
    #Foreign Key columns for artist and venue
...
def venues():
    ...
    # Get data on venues and populate data list
    ...
    #iterate and populate data dictionary with locations
    for loc in cities_states:
        # see if there are any venues in this location, and add if so
        venues_list = []
        for venue in venues:
            if (venue.city == loc[0]) and (venue.state == loc[1]):

                # check how many upcoming shows a venue has
...
def search_venues():
    ...
    for venue in venues:
        ...
        venue_list.append({
            "id": venue.id,
            "name": venue.name,
            "num_upcoming_shows": num_upcoming  # template does nothing with this
        })
...
def show_venue(venue_id):
    ...
    print(venue)
    if not venue:
        # Didn't return one, Redirect home
...
def delete_venue(venue_id):
    ...
    if not venue:
        ...
        # hang on to venue name since will be lost after delete
...
def edit_artist(artist_id):
    ...
    artist = Artist.query.get(artist_id)  # Returns object based on primary key, or None. 
    if not artist:
        # User typed in a URL that doesn't exist, redirect home
        return redirect(url_for('index'))
    else:
        # Otherwise, valid artist.
        # Prepopulate the form with the current values.  
